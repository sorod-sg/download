...
func (d *Downloader) Download(URL, filename string) error {
	...
	return d.singDownload(URL, filename)
}
func (d *Downloader) multiDownload(URL, filename string, contentlen int) error {
	...
	fmt.Println(d.concurrency)
	rangstart := 0

	for i := 0; i < d.concurrency; i++ {
		go func(i, ranstart int) {
			defer wg.Done()
			rangeEnd := ranstart + partSize
			if i == d.concurrency-1 {
				rangeEnd = contentlen
			}
			d.downloadPartial(URL, filename, rangstart, rangeEnd, i)
		}(i, rangstart)
